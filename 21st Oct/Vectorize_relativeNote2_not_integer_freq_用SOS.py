...
def Bfilter(data,wn=20,hn=20000):
    sos = signal.butter(8, [0.02,0.17], 'bandpass', output='sos')   #配置滤波器 8 表示滤波器的阶数

    filtedData = signal.sosfilt(sos, data)  #data为要过滤的信号
    #print(filtedData)
    return filtedData
...
def get_note(file_name='01-D_AMairena.wav'):
    ...
    for i in range(len(on_set)-1):
        ...
        pitch_number=np.mean(f0[on_set[i]:on_set[i+1]])#Counter(f0[on_set[i]:on_set[i+1]]).most_common(1)[0][0]
        divide_number=1
...
def dtw_distance(ts_a, ts_b, d=lambda x,y: abs(x-y), mww=10000):
    ...
    cost[1:M,0]=cost[0:M-1,0]+d(ts_a[1:M],np.array([ts_b[0]]*(M-1)))
    #for i in range(1, M):
    #    cost[i, 0] = cost[i-1, 0] + d(ts_a[i], ts_b[0])

    cost[0,1:N]=cost[0,0:N-1]+d(np.array([ts_a[0]]*(N-1)),ts_b[1:N])
    #for j in range(1, N):
    #    cost[0, j] = cost[0, j-1] + d(ts_a[0], ts_b[j])

    # Populate rest of cost matrix within window
    #cost[1:M,max(1, i - mww): min(N, i + mww)]
    for i in range(1, M):
...
def Levenshtein_Distance(str1, str2):
    ...
    for i in range(1, len(str1)+1):
        for j in range(1, len(str2)+1):
            matrix[i][j] = min(matrix[i-1][j]+1, matrix[i][j-1]+1, matrix[i-1][j-1]+int(not str1[i-1] == str2[j-1]))
...
while number<=50:
    if number<10:
        a=get_note("CSD/english/wav/en00"+str(number)+"a.wav")
    else:
        a=get_note("CSD/english/wav/en0"+str(number)+"a.wav")
    a=relative_pitch(a)
    see_files=50
    distance=np.zeros(see_files+1)
    for w in range(1,see_files+1):
        if w<10:
        ...
        for j in range(0,len(notes)):
            if notes[j].end-notes[0].start>=Time_length:
                end_index=j
                break
        b=get_midi_inf(notes[0:end_index],hop_length=1024)
        b=relative_pitch(b)
